...
int ReplicatedPG::do_replica_safe_read(
  OSDOp &osd_op,
...
{
  int result = 0;
  SnapSetContextRef ssc = ctx->obc->ssc;
  ObjectState& obs = ctx->new_obs;
  ...
  for (vector<OSDOp>::iterator p = ops.begin(); p != ops.end(); ++p, ctx->current_osd_subop_num++) {
   ...
   case CEPH_OSD_OP_NOTIFY:
	...
	if (pool.info.require_rollback()) {
	  ...
	  bufferlist old;
	  int r = getattr_maybe_cache(soid, ctx->obc, name, &old);
	  if (r == 0) {
	    to_set[name] = old;
	  } else {
	    to_set[name];
 ...
 */
bool ReplicatedPG::validate_snap_mapping(
  snapid_t snap,
  const SnapSet &ss,
...
SnapSetContextRef ReplicatedPG::get_snapset_context(
  const hobject_t& oid,
  bool can_create,
  bufferlist *attr,
  bool oid_existed)
  ...
  bufferlist *val)
{
  ...
  return pgbackend->objects_get_attr(soid, key, val);
}
